...
//		test.testCalibrate();
		...
		for( int j=0; j<writes; j++ ) {
			for( int i=0; i<samplesPerWrite; i++) {
				...
				for (int k=0; k<reader.getScaleCorrection().length; k++) {
					LCD.drawString( String.format("c: %2.3f / %2.3f", reader.getScaleCorrection()[k] , reader.getOffsetCorrection()[k]), 0, k+1);
				}
				readerArm.rotate(-READ_ANGLE);
			}
			write1();
	...
	/**
	 * Reads the current bit on the tape.
	 * Moves the reader arm in, reads MEAN_SIZE samples and feeds it through the linear calibration filter to normalize to (-1,1).
